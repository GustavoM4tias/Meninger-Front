...
<template>
  <div class="fixed top-0 left-0 w-full h-screen bg-gray-800 bg-opacity-75 flex justify-center items-center z-50">
    <div class="relative w-[300px] sm:w-[600px] bg-gray-200 dark:bg-gray-700 pt-10 px-4 m-3 rounded-2xl">
      <i class="fas fa-xmark absolute top-0 left-0 p-3 text-4xl cursor-pointer hover:text-gray-600 dark:hover:text-gray-400" @click="cancelEditing"></i>
      <form @submit.prevent="saveUser">
        ...
        <div class="flex gap-3">
          <Button type="submit" class="bg-emerald-400 hover:bg-emerald-500 dark:bg-emerald-500 dark:hover:bg-emerald-600">Salvar</Button>
        </div>
      </form>
    </div>
  </div>
