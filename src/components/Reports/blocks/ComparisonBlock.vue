...
// Comparativo com um lado só não compara nada.
const incompleto = computed(() => (props.sides || []).length < 2)
const legenda = (s) => inlineMd(s?.caption)
</script>

<template>
  <BlockEmpty v-if="incompleto" :label="title || 'Comparativo'" hint="Faltam os dois lados da comparação." icon="fas fa-scale-balanced" />
