  ...
  <ul v-if="users.length">
    <li v-for="user in users" :key="user.id">
      <div class="relative bg-gray-200 dark:bg-gray-600 flex justify-between items-center lg:mx-48 m-4 rounded-2xl p-4 md:px-8 shadow">
        ...
        <p class="absolute top-3.5 left-0 md:w-20 rounded-br-xl rounded-tr-sm text-white shadow-sm text-center text-xs md:text-base font-semibold" :class="{ 'bg-green-400 dark:bg-green-500': user.status === true, 'bg-red-400 dark:bg-red-500': user.status === false }">{{ user.status === true ? 'Ativo' : 'Inativo' }}</p> 
