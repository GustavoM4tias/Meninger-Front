...
const emit = defineEmits(['close', 'building-deleted']);
...
const handleBuildingDeleted = () => {
    emit('building-deleted'); // Emite o evento para `Buildings.vue`
};
...
const excluirBuilding = async () => {
    ...
    if (confirmar === true) {
        try {
            ...
            emit('building-deleted'); // Emite o evento de exclusão
        ...
        <div class="bg-gray-700 sm:h-5/6 sm:w-8/12 w-11/12 rounded-lg mx-auto relative">
            <div class="content h-full grid grid-cols-1 lg:grid-cols-3">
                <div class="img col-span-1 sm:col-span-2 relative rounded-t-lg lg:rounded-l-lg lg:rounded-tr-none h-100 w-full h-full overflow-hidden">
                    <img :src="building.images[imagemAtual] || '/noimg.jpg'" class="w-full h-full object-cover" />

                    <div class="absolute filter drop-shadow top-0 left-0 m-5">
                        <h2 class="text-2xl md:text-3xl font-bold text-gray-50 mb-2 mr-5 drop-shadow-xl">{{ building.title }}</h2>
                ...
                <div class="text flex flex-col">
                    <div class="text text-gray-100 flex flex-col py-4 px-4 md:px-6">
                        ...
                        <span class="font-normal">{{ new Date(building.event_date).toLocaleDateString() }}</span>
                        <div class="descricao text-gray-300 mb-6">
                            <p class="pl-2">Descrição:</p>
                            <p class="text-md border rounded-xl  md:rounded-2xl p-2 md:p-3">{{ building?.description }}</p>
                        </div>
