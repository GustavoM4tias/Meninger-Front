...
// w-64 = 16rem | w-12 = 3rem (ainda menor e acompanha só ícones)
const sidebarWidthClass = computed(() => (isCollapsed.value ? 'sm:w-12 w-12' : 'sm:w-64 w-64'));
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center justify-start rtl:justify-end">
            ...
            <a href="" @click.prevent="toggleSidebar" class="flex ms-2 md:me-24 cursor-pointer select-none">
              <img
                src="/Mlogotext.png"
                class="h-10 sm:h-12 -my-4 dark:invert-0 invert"
                alt="Menin Logo"
              />
            </a>
          </div>
      ...
      >
        <ul class="space-y-2 font-medium overflow-auto">
          ...
          <li>
            <button
              type="button"
              @click="toggleDropdownSafe('favorites')"
            ...
            >
              ...
              <span v-show="!isCollapsed" class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap transition-opacity duration-200 ease-in-out">
                Favoritos
              </span>
              ...
              <i
                ...
                class="text-gray-500 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white transition-opacity duration-200 ease-in-out"
              ></i>
            </button>
                ...
                <li v-for="(subGroups, category) in groupedFavorites" :key="category" class="ml-4">
                  ...
                  <ul>
                    <li v-for="(favorites, subcategory) in subGroups" :key="subcategory" class="ml-2">
                      <p v-if="subcategory !== '__sem_subcategoria__'" class="text-sm text-gray-500 dark:text-gray-400 mb-1">
                        {{ subcategory }}
                      </p>
                      <ul class="space-y-1">
          ...
          <li v-for="catKey in categoryKeys" :key="catKey">
            <button
              type="button"
              @click="toggleDropdownSafe(catKey)"
            ...
            >
              <i
                ...
                class="w-5 text-gray-500 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white"
              ></i>
            ...
            <ul v-show="!isCollapsed && dropdowns[catKey]" class="py-2 space-y-2 ml-4">
              ...
              <template v-if="!isArrayCategory(catKey)">
                <li v-for="[subKey, subObj] in subcatEntries(catKey)" :key="`${catKey}.${subKey}`">
                  <button
                    type="button"
                    @click="toggleSubDropdownSafe(catKey, subKey)"
                  ...
                  >
                    <i
                      :class="subObj.icon || 'far fa-folder'"
                      class="w-5 text-gray-500 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white"
                    ></i>
                  ...
                  <ul v-show="!isCollapsed && subDropdowns[`${catKey}.${subKey}`]" class="py-1 space-y-1 ml-4">
                    <li v-for="item in subObj.items" :key="`${item.router}-${item.section}`">
                      <div class="flex items-center justify-between group/item hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg px-2 py-1">
                        ...
                        <button
                          @click="toggleFavorite(item.router, item.section)"
                        ...
                        >
                          <i :class="isFavorited(item.router, item.section) ? 'fas fa-star' : 'far fa-star'" class="text-lg"></i>
                        </button>
                      </div>
                    </li>
                  </ul>
