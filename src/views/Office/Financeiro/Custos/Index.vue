    ...
    <div class="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div class="rounded-2xl border dark:border-gray-700 bg-white/90 dark:bg-gray-800 shadow-lg overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-900/40">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ filteredGroups.length }} empreendimento(s)
                    </div>
                </div>
            </div>
        ...
        <Teleport to="body">
            <div v-if="selectedGroup"
                ...
                <!-- Panel -->
                <div class="relative w-full max-w-7xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="text-xl font-bold flex items-center gap-2">
                                    <i class="fas fa-building"></i>
                                    {{ resolveEnterpriseName(selectedGroup.costCenterId) || selectedGroup.costCenterName
                                    ||
                                    'Empreendimento' }}
                                </h3>
                                <div class="flex flex-wrap items-center gap-3 mt-2 text-sm text-emerald-50">
                                    <span><i class="fas fa-hashtag text-xs mr-1"></i>CC {{ selectedGroup.costCenterId
                                        }}</span>
                                    <span class="opacity-50">|</span>
                                    <span><i class="fas fa-list-ul text-xs mr-1"></i>{{ modalExpenses.length }} de {{
                                        selectedGroup.expenses.length }} lançamento(s)</span>
                                    <span class="opacity-50">|</span>
                            ...
                            <button @click="closeDetails"
                                class="flex-none p-2 rounded-xl bg-white/20 hover:bg-white/30 transition-colors">
                    ...
                    <div
                        class="flex-none border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 px-6 py-4 space-y-3">
                        ...
                        <div class="flex flex-col md:flex-row gap-3 items-stretch md:items-center">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="relative">
                                    ...
                                    <button v-if="modalSearch" @click="modalSearch = ''"
                            ...
                            <div class="w-full md:w-44">
                                <select v-model.number="modalFilterCat"
                                    ...
                                    <option v-for="cat in categoryOptions" :key="cat.id" :value="cat.id">{{ cat.name }}
                                    </option>
                                </select>
                            </div>
                            ...
                            <!-- Sem categoria -->
                            <label
                                class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap cursor-pointer select-none px-1">
                            ...
                            <!-- Limpar todos -->
                            <button v-if="hasModalFilters" @click="clearModalFilters"
                        ...
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                            <span
                                class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap flex items-center gap-1.5">
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                <div class="flex items-center gap-2">
                                    <select v-model.number="bulkCategoryId"
                                        ...
                                        <option v-for="cat in categoryOptions" :key="cat.id" :value="cat.id"
                                            class="text-gray-800">{{ cat.name }}</option>
                                    </select>
                                ...
                                <div class="flex items-center gap-2">
                                    <select v-model="bulkDepartment"
                                        ...
                                        <option v-for="d in store.departmentOptions" :key="d" :value="d"
                                            class="text-gray-800">
                                            {{ d }}</option>
                                    </select>
                            ...
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th @click="handleModalSort('date')"
                                        class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-900 dark:hover:text-white whitespace-nowrap">
                                        <span class="flex items-center gap-1">Vencimento <i
                                                :class="getModalSortIcon('date')"></i></span>
                                    </th>
                                    <th @click="handleModalSort('title')"
                                        class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-900 dark:hover:text-white">
                                        <span class="flex items-center gap-1">Fornecedor / Título <i
                                                :class="getModalSortIcon('title')"></i></span>
                                    </th>
                                    <th
                                        class="px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">
                                        Parcela</th>
                                    <th @click="handleModalSort('amount')"
                                        class="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-900 dark:hover:text-white whitespace-nowrap">
                                        <span class="flex items-center justify-end gap-1">Valor <i
                                                :class="getModalSortIcon('amount')"></i></span>
                                    </th>
                                    <th
                                        class="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">
                                        V. Título</th>
                                    <th @click="handleModalSort('department')"
                                        class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-900 dark:hover:text-white">
                                        <span class="flex items-center gap-1">Departamento <i
                                                :class="getModalSortIcon('department')"></i></span>
                                    </th>
                                    <th @click="handleModalSort('category')"
                                        class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-900 dark:hover:text-white">
                                        <span class="flex items-center gap-1">Categoria <i
                                                :class="getModalSortIcon('category')"></i></span>
                                    </th>
                                    <th
                                        class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider min-w-[80px]">
                                        Observação</th>
                                    <th
                                ...
                                <tr v-for="exp in modalExpenses" :key="exp.id"
                                    ...
                                    <td class="px-4 py-3 max-w-[240px]">
                                        <div v-if="exp.bill">
                                            ...
                                            <div v-if="exp.bill.notes" class="text-[10px] text-gray-400 truncate mt-0.5"
                                    ...
                                    <!-- Valor total título -->
                                    <td
                                        class="px-4 py-3 whitespace-nowrap text-right text-xs text-gray-500 dark:text-gray-400">
                                        <span v-if="exp.bill?.totalInvoiceAmount">
                                            {{ Number(exp.bill.totalInvoiceAmount).toLocaleString('pt-BR', {
                                                style:
                                            'currency',
                                            currency: 'BRL' }) }}
                                        </span>
                                    ...
                                    <td class="px-4 py-3 max-w-20 whitespace-nowrap">
                                        <span v-if="exp.departmentName || exp.bill?.mainDepartmentName"
                                            ...
                                            <p class="truncate mx-auto">{{ exp.departmentName ||
                                    ...
                                    <td class="px-4 py-3">
                                        <div v-if="exp.description"
                                            class="text-xs text-gray-700 dark:text-gray-300 truncate max-w-[80px]"
                    ...
                    <!-- ── Footer ── -->
                    <div
                        class="flex-none px-6 py-4 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-200 dark:border-gray-700">
                        <div class="flex items-center justify-between gap-3">
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                Mostrando {{ modalExpenses.length }} de {{ selectedGroup.expenses.length }} lançamentos
                                ·
                                Total filtrado: <span class="font-semibold text-emerald-600">{{
                                    modalTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }}</span>
                            </div>
                            <button @click="closeDetails"
                ...
                <div class="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden">
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-blue-100 mt-0.5">
                                    {{ editingExpense.bill?.creditor_json?.tradeName ||
                                        editingExpense.bill?.creditor_json?.name
                                    || editingExpense.description || '#' + editingExpense.id }}
                                </p>
                            </div>
                    ...
                    <div class="px-6 py-4 bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
                        <div class="grid grid-cols-2 gap-3 text-xs text-gray-600 dark:text-gray-400">
                            <div>
                                <span
                                    class="font-semibold text-gray-700 dark:text-gray-300 block mb-0.5">Vencimento</span>
                                {{ formatDate(editingExpense.dueDate) }}
                                <span class="text-gray-400 block">Competência: {{
                                    formatMonth(editingExpense.competenceMonth)
                                    }}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                <span
                                    class="font-semibold text-gray-700 dark:text-gray-300 block mb-0.5">Documento</span>
                                {{ editingExpense.bill?.document_identification_id }} {{
                                    editingExpense.bill?.document_number ||
                                '—' }}
                            </div>
                            <div>
                                <span class="font-semibold text-gray-700 dark:text-gray-300 block mb-0.5">V.
                                    Título</span>
                                {{ editingExpense.bill?.totalInvoiceAmount
                                    ? Number(editingExpense.bill.totalInvoiceAmount).toLocaleString('pt-BR', {
                                        style:
                                            'currency',
                                currency: 'BRL' })
                                : '—' }}
                    ...
                    <div class="px-6 py-5 space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span
                                    class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm font-medium">R$</span>
                        ...
                        <div>
                            ...
                            <select v-model="editForm.departmentName"
                                ...
                                <option v-for="dep in store.departmentOptions" :key="dep" :value="dep">{{ dep }}
                                </option>
                            </select>
                        </div>
                    ...
                    <!-- Footer -->
                    <div
                        class="px-6 py-4 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
                        <button @click="closeEditModal"
