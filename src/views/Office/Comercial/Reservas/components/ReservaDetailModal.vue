          ...
          <div class="space-y-5">
            ...
            <section>
              <div class="flex items-center gap-1.5 mb-2">
                ...
                <h4 class="text-micro font-mono uppercase tracking-wider text-ink-subtle">Status</h4>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                ...
                <div class="rounded-lg p-2.5 border border-line bg-surface-sunken"
                  ...
                  <p class="text-micro uppercase tracking-wider text-ink-subtle font-mono">Etapa Vendida (CRM)</p>
                  <p class="text-sm font-semibold mt-0.5"
                    :class="isVendida ? 'text-data-pos' : 'text-ink-subtle'">
                    {{ isVendida ? 'Sim' : 'Não' }}
                  </p>
                </div>
              </div>
            ...
            <section>
              <div class="flex items-center gap-1.5 mb-2">
                ...
                <h4 class="text-micro font-mono uppercase tracking-wider text-ink-subtle">Imóvel e responsáveis</h4>
              </div>
            ...
            <section>
              <div class="flex items-center gap-1.5 mb-2">
                ...
                <h4 class="text-micro font-mono uppercase tracking-wider text-ink-subtle">Datas e referências</h4>
              </div>
              <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
                <div class="rounded-lg p-2.5 border border-line bg-surface-sunken">
                  <p class="text-micro uppercase tracking-wider text-ink-subtle font-mono">Cadastro reserva</p>
                  <p class="text-sm text-ink font-mono">{{ fmt(reserva?.data_reserva) }}</p>
                </div>
                <div class="rounded-lg p-2.5 border border-line bg-surface-sunken">
                  <p class="text-micro uppercase tracking-wider text-ink-subtle font-mono">Data contrato</p>
                  <p class="text-sm text-ink font-mono">{{ fmt(reserva?.data_contrato) }}</p>
                </div>
                <div class="rounded-lg p-2.5 border border-line bg-surface-sunken"
                  v-tippy="'Data em que a etapa Vendida foi marcada no CRM'">
                  <p class="text-micro uppercase tracking-wider text-ink-subtle font-mono">Etapa Vendida</p>
                  <p class="text-sm font-mono"
                    :class="reserva?.data_venda ? 'text-data-pos font-semibold' : 'text-ink'">
                    {{ fmt(reserva?.data_venda) }}
                  </p>
                </div>
                <div class="rounded-lg p-2.5 border border-accent/30 bg-accent-soft">
                  <p class="text-micro uppercase tracking-wider text-accent font-mono">Dias em reserva</p>
                  <p class="text-sm font-bold text-accent tabular-nums">
                    {{ Number(reserva?.dias_em_reserva || 0).toFixed(1) }}d
                  </p>
                </div>
                <div class="rounded-lg p-2.5 border border-line bg-surface-sunken">
                  <p class="text-micro uppercase tracking-wider text-ink-subtle font-mono">Pré-cadastro #</p>
                  <p class="text-sm text-ink font-mono">{{ reserva?.idprecadastro || '-' }}</p>
                </div>
                <div class="rounded-lg p-2.5 border border-line bg-surface-sunken">
                  <p class="text-micro uppercase tracking-wider text-ink-subtle font-mono">Proposta CV</p>
                  <p class="text-sm text-ink font-mono">{{ reserva?.idproposta_cv || '-' }}</p>
                </div>
                <div class="rounded-lg p-2.5 border border-line bg-surface-sunken">
                  <p class="text-micro uppercase tracking-wider text-ink-subtle font-mono">Proposta interna</p>
            ...
            <section v-if="reserva?.observacoes">
              <div class="flex items-center gap-1.5 mb-2">
                ...
                <h4 class="text-micro font-mono uppercase tracking-wider text-ink-subtle">Observações</h4>
              </div>
        ...
        <template v-else-if="tab === 'contratos'">
          ...
          <div v-else class="space-y-2.5">
            <div v-for="(c, i) in contratos" :key="i"
              class="rounded-xl border border-line bg-surface-sunken p-3 border-l-4 border-l-data-pos">
        ...
        <template v-else-if="tab === 'mensagens'">
          ...
          <div v-else class="space-y-2.5">
            <div v-for="(m, i) in mensagens" :key="i"
              ...
              <div class="text-micro font-mono text-ink-subtle mb-1">{{ fmt(m.data_cad || m.data) }}</div>
