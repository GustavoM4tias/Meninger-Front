<template>
  <div class="space-y-4">
    ...
    <div
      ...
      <div class="px-5 py-3.5 border-b border-line bg-surface-sunken">
        <p class="lbl-section"><i class="fas fa-hand-holding-dollar text-accent"></i> Parcelas e Entrada</p>
      </div>
  ...
  class="inp-pfx" placeholder="0,00" :disabled="readonly" />
    ...
    <div
      class="bg-surface-raised rounded-2xl border border-line shadow-sm overflow-hidden">
      <div class="px-5 py-3.5 border-b border-line bg-surface-sunken">
        <p class="lbl-section"><i class="fas fa-calendar-days text-accent"></i> Reajuste e Prazo</p>
      </div>
    ...
    <div
      class="bg-surface-raised rounded-2xl border border-line shadow-sm overflow-hidden">
      <div class="px-5 py-3.5 border-b border-line bg-surface-sunken">
        <p class="lbl-section"><i class="fas fa-building-flag text-accent"></i> Subsídio Estadual</p>
      </div>
      <div class="p-5 space-y-4">

        <!-- Toggle -->
        <div
          class="flex items-start justify-between gap-6 p-4 rounded-xl border border-line bg-surface-sunken">
          <div>
              ...
              <div class="flex gap-2 flex-wrap mt-1">
                <label v-for="opt in subsidyStates" :key="opt.value"
                  ...
                  <span
                    ...
                    :class="form.state_subsidy_state === opt.value ? 'border-accent bg-accent' : 'border-line'">
                    <span v-if="form.state_subsidy_state === opt.value" class="w-1 h-1 rounded-full bg-surface-raised"></span>
                  </span>
                  {{ opt.label }}
                </label>
              </div>
...
.inp {
  @apply w-full px-3.5 py-2.5 text-sm text-ink bg-surface-raised/60 border border-line rounded-md shadow-sm placeholder:text-ink-subtle dark:placeholder:text-ink-muted outline-none focus:border-accent dark:focus:border-accent focus:ring-accent/15 transition-all duration-150;
}

.inp-pfx {
  @apply w-full pl-9 pr-3.5 py-2.5 text-sm text-ink bg-surface-raised/60 border border-line rounded-md shadow-sm placeholder:text-ink-subtle dark:placeholder:text-ink-muted outline-none focus:border-accent dark:focus:border-accent focus:ring-accent/15 transition-all duration-150;
}
